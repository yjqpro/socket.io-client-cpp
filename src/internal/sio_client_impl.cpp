...
void client_impl::connect(const string& uri,
...
void client_impl::on_socket_closed(string const& nsp) {
  if (m_socket_close_listener)
    m_socket_close_listener(nsp);
}

void client_impl::on_socket_opened(string const& nsp) {
  if (m_socket_open_listener)
    m_socket_open_listener(nsp);
}
...
std::string client_impl::encode_query_string(const std::string& query) {
  ...
  for (const char c : query) {
    if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') ||
      ...
      ss << '%' << std::uppercase << std::setw(2) << int((unsigned char)c)
         << std::nouppercase;
    }
  }
  ss << std::dec;
